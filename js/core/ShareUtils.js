...
const SHARE_UTILS_NPC_DEFINITIONS = (() => {
    if (typeof window !== 'undefined' && window.NPCDefinitions) {
        return window.NPCDefinitions.definitions
...
function resolveNpcType(npc) {
    ...
    if (typeof npc?.id === 'string') {
        const matchById = SHARE_UTILS_NPC_DEFINITIONS.find((def) => def.id === npc.id);
        if (matchById) return matchById.type;
    }
    if (typeof npc?.name === 'string') {
        const matchByName = SHARE_UTILS_NPC_DEFINITIONS.find((def) => def.name === npc.name);
...
function normalizeSprites(list) {
    ...
    for (const npc of list) {
        ...
        const def = SHARE_UTILS_NPC_DEFINITIONS.find((entry) => entry.type === type);
        if (!def) continue;
...
function encodeNpcTypeIndexes(sprites) {
    ...
    for (let i = 0; i < sprites.length; i++) {
        ...
        const index = SHARE_UTILS_NPC_DEFINITIONS.findIndex((def) => def.type === sprite.type);
...
function decodeShareCode(code) {
    ...
    const canUseDefinitions = SHARE_UTILS_NPC_DEFINITIONS.length > 0 && (npcTypeIndexes.length > 0 || npcPositions.length <= SHARE_UTILS_NPC_DEFINITIONS.length);
    const sprites = [];
    if (canUseDefinitions) {
        for (let index = 0; index < npcPositions.length; index++) {
            const typeIndex = npcTypeIndexes[index] ?? index;
            const def = SHARE_UTILS_NPC_DEFINITIONS[typeIndex];
            if (!def) continue;
...
static buildShareUrl(gameData) {
    return buildShareUrl(gameData);
}

static extractGameDataFromLocation(location) {
    return extractGameDataFromLocation(location);
}

static encode(gameData) {
    return buildShareCode(gameData);
}
